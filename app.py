...
def predict(review_text):
    ...
    if prediction == 0:
        pred["label"] = "La reseña es negativa"
        pred["score"] = f"{torch.softmax(output, dim=1)[0][0].item()*100:.2f}%"
    else:
        pred["label"] = "La reseña es positiva"
...
tienda_app = gr.Interface(
    fn=predict,
    inputs=gr.Textbox(label="Introduce la reseña", placeholder="Escribe aquí..."),
    outputs=[gr.Label(label="Predicción"), gr.Label(label="Calificación")],
    title="IA & BIGDATA: Detección de reseñas positivas y negativas",
    description="Introduce la reseña",
    #layout="vertical",
